...
class RNNEncoder(nn.Module):
    ...
    def forward(self, ids, lengths, word_embeddings, hidden):
        ...
        # matrix of word embeddings. sentence size * batch size * embedding dims (150)
        if is_varlen:
            embeddings = nn.utils.rnn.pack_padded_sequence(embeddings, lengths)
        output, hidden = self.rnn(embeddings, hidden)
        # hidden should be (bidirectional *layers) * batch size * hidden size/2 (because bidirectional) (301)
        # output should be sentence size * batch size * hidden size (602)
